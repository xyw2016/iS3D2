...
particle_list_file = 'particle_list_osc.dat'
...
with open(particle_list_file, 'r') as file:
    ...
    for line in lines:
        ...
        if len(line_split) != 4 and len(line_split) != 11:
            continue
        ...
        if len(line_split) == 4:
            ...
            if current_particle < num_particles:
                ...
                mcid = int(line_split[1])
                energy = float(line_split[5])
                px = float(line_split[2])
                py = float(line_split[3])
                pz = float(line_split[4])
